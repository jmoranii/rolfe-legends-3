...
- **SECRET (zero-hint — never referenced in any visible UI, copy, tip, or Barn Book slot until unlocked):** **Goldie the llama is the secret LEGENDARY PET.** Unlock ritual, deliberately obscure (James: "really hard to find"): once all three duck super-pets live in the barn, pat the ducks **in world order — Brownie, Diver, Harmless — consecutively in a single barn visit** (any other pat breaks the chain, silently). Goldie then wanders to the farm gate; tap her **3×** (RL1/RL2 canon) and she joins the barn. Kit: companion action *Llama Stare* (a random enemy loses its nerve — Weak), signature cards *SPIT!* (hits ALL enemies) and *Goldie Knows* (look at the top 3 cards of your draw pile, reorder them). **No visual tell of any kind** — the RL2 white-dot lesson is canon: no hotspot artifact, no cursor change, no distinct DOM node, no render difference at the gate pre-unlock; an e2e audit asserts this. Her Barn Book entry doesn't exist pre-unlock (no silhouette — the Book shows a count that excludes her).
